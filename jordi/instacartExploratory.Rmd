...
```{r}
dbGetQuery(sc, "SELECT * FROM order_products__prior_tbl LIMIT 20")
```
...
```{r}
dia<-dbGetQuery(sc, "SELECT order_dow,COUNT(order_dow) AS num FROM orders_tbl GROUP BY order_dow  ORDER BY num DESC")
dia
hora<-dbGetQuery(sc, "SELECT order_hour_of_day,COUNT(order_hour_of_day) AS num FROM orders_tbl GROUP BY order_hour_of_day  ORDER BY num DESC")
hora
reor<-dbGetQuery(sc, "SELECT product_id,COUNT(reordered) AS num FROM order_products__train_tbl GROUP BY product_id  ORDER BY num DESC")
reor
dbGetQuery(sc, "SELECT order_id,COUNT(1) AS num FROM order_products__prior_tbl GROUP BY order_id  ORDER BY num DESC")
```
## Whisker, magrittr
